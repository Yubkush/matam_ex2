...
const std::string FILE_PATH = "../cppTestsBiadsy/local/testOutputsBiadsy";
...
bool testSkill() {
    std::string fileName = FILE_PATH + std::string("/printed/testSkill.txt");
    std::ofstream out(fileName, std::ofstream::trunc | std::ofstream::in);
    if(!(out).is_open()){
        throw FileFailed();
    }
